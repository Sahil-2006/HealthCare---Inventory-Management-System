...
function Shell({ active, onNavigate, children, menuOpen, setMenuOpen, snapshotAt, dateLabel }) {
  ...
  return <div className="workspace-shell">
    <aside className={`workspace-sidebar ${menuOpen ? 'open' : ''}`} aria-label="primary navigation">
      ...
      <div className="sidebar-footer"><p><i />network snapshot</p><small>8 facilities connected</small><button className="guide"><span>?</span>workspace guide</button><div className="profile"><div>DA</div><p><strong>demo approver</strong><small>approver account</small></p></div></div>
    </aside>
    {menuOpen && <button className="sidebar-backdrop" aria-label="close navigation" onClick={() => setMenuOpen(false)} />}
    <main className="workspace-main">
      <header className="workspace-topbar"><button className="menu-open" aria-label="open navigation" onClick={() => setMenuOpen(true)}><Icon name="menu" /></button><p>workspace <Icon name="chevron" size={12} /> <strong>{current}</strong></p><div><span className="top-pill"><i />demo workspace</span><span className="top-pill">{dateLabel || 'live fixture'}</span></div></header>
...
function Plan({ data, onDecision, decisionBusy, onNavigate }) {
  ...
  return <>
    ...
    <div className="plan-grid"><Card className="instruction-card"><p className="eyebrow">safe sourcing route</p><h2>instruction set</h2><p className="muted">{data.rationale}</p>{data.transfers.map((transfer, index) => <article className="transfer" key={transfer.source}><b>{index + 1}</b><div><span>source {index + 1}</span><h3>{transfer.source}</h3><p>deliver {transfer.quantity} units of {data.medicine}</p><small>{transfer.constraint}</small></div><div><strong>{transfer.quantity}<small> units</small></strong><span>{transfer.distance}</span><small>remaining safe stock <b>{transfer.remaining} units</b></small></div></article>)}</Card><Card className="plan-summary"><h2>execution summary</h2><dl><div><dt>total sourced</dt><dd>{data.summary.total} units</dd></div><div><dt>new projected life</dt><dd className="good">{data.summary.projectedLife}</dd></div><div><dt>uncertainty factor</dt><dd className="watch-text">{data.summary.uncertainty}</dd></div><div><dt>new stockouts</dt><dd>{data.summary.noNewStockouts ? 'none projected' : 'review required'}</dd></div></dl><label>approval note <small>optional</small><textarea value={note} disabled={settled} onChange={(event) => setNote(event.target.value)} placeholder="add an operational note for the audit trail" /></label>{settled ? <div className={`decision ${data.status}`}><Icon name={data.status === 'approved' ? 'check' : 'close'} /><strong>plan {data.status}</strong><span>the decision was appended to the audit trail.</span></div> : <div className="plan-actions"><Button primary disabled={decisionBusy} onClick={() => onDecision('approved', note)}>{decisionBusy ? 'recording decision…' : 'approve transfers ✓'}</Button><Button disabled={decisionBusy} className="danger" onClick={() => onDecision('rejected', note)}>reject and re-route</Button></div>}<button className="record-link" onClick={() => onNavigate('audit')}>view decision record →</button></Card></div>
  </>;
}
...
function App() {
  ...
  const page = useMemo(() => {
    ...
    if (view === 'plan') return <Plan data={payload.plan} decisionBusy={decisionBusy} onDecision={decide} onNavigate={setView} />;
  ...
  if (loading) return <div className="app-state"><Icon name="ripple" size={28} /><strong>loading medripple</strong><span>preparing your regional resilience snapshot…</span></div>;
  if (error) return <div className="app-state"><EmptyOrError title="regional workspace unavailable" copy={error} retry={load} /></div>;
  return <><Shell active={view} onNavigate={setView} menuOpen={menuOpen} setMenuOpen={setMenuOpen} snapshotAt={payload.dashboard.snapshotAt} dateLabel={payload.dashboard.dateLabel}>{page}</Shell>{toast && <div className="toast"><Icon name="check" size={15} />{toast}<button onClick={() => setToast('')} aria-label="dismiss"><Icon name="close" size={14} /></button></div>}</>;
}
